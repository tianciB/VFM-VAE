...
#   https://github.com/FoundationVision/UniTok/blob/main/models/quant.py
#
# The original implementation is part of the UniTok project by FoundationVision.
# ------------------------------------------------------------------------------
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, features):
        ...
        handler = torch.distributed.all_reduce(prob_per_class_is_chosen, async_op=True) if (
                self.training and dist.is_initialized()) else None
        if handler is not None:
            handler.wait()
        
        # better readibility
        # prob_per_class_is_chosen = indices.bincount(minlength=self.vocab_size).float().to(features.device)
        # if self.training and dist.is_initialized():
        #     torch.distributed.all_reduce(prob_per_class_is_chosen, op=torch.distributed.ReduceOp.SUM)
