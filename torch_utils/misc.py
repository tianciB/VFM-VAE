    ...
    for name, tensor in named_params_and_buffers(dst_module):
        ...
        if name in src_tensors:
            try:
                ...
                raise RuntimeError(
                    f'Failed to copy tensor "{name}" from module {type(src_module).__name__} to {type(dst_module).__name__}. '
                    f'Original error: {e}'
                ) from e
        else:
            print(f'Warning: {type(dst_module).__name__}.{name} is not present in the source module.')
