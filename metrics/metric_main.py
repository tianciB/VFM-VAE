...
def report_metric(result_dict: dict, run_dir: Optional[str] = None, snapshot_path: Optional[str] = None) -> None:
    """
    Record metric results into JSONL logs.

    Args:
        result_dict: A dict (or EasyDict) returned from calc_metric().
        run_dir: Output directory of current training run.
        snapshot_path: Path to the model checkpoint (.pt or .pkl).
    """
    metric = result_dict['metric']
    assert is_valid_metric(metric)

    # If both run_dir and snapshot_path exist, store relative path.
    rel_snapshot = None
    if run_dir is not None and snapshot_path is not None:
        rel_snapshot = os.path.relpath(snapshot_path, run_dir)

    # For backward compatibility.
    jsonl_line = json.dumps(
        dict(result_dict, snapshot_path=rel_snapshot, timestamp=time.time()),
        ensure_ascii=False
    )

    print(jsonl_line)
    if run_dir is not None and os.path.isdir(run_dir):
        out_file = os.path.join(run_dir, f'metric-{metric}.jsonl')
