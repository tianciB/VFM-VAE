...
#   https://github.com/minyoungg/platonic-rep/blob/main/measure_alignment.py
...
def load_and_process_features(feat_path1, feat_path2):
    """
    Load and process features from two .pt files
    """
    print(f"Loading features from:")
    print(f"  File 1: {feat_path1}")
    print(f"  File 2: {feat_path2}")
    
    # Load features
    data1 = torch.load(feat_path1)
    data2 = torch.load(feat_path2)
    
    feats_A = data1['features']
    feats_B = data2['features']
    
    print(f"Original feature shapes: {feats_A.shape}, {feats_B.shape}")
    
    # Prepare features (remove outliers)
    print("Preparing features...")
    feats_A = prepare_features(feats_A, q=0.95, exact=True)
    feats_B = prepare_features(feats_B, q=0.95, exact=True)
...
def compute_all_metrics(feats_A, feats_B, trials=10):
    """
    Compute all supported alignment metrics with multiple trials
    """
    import time
    import numpy as np
    
    results = {}
    print(f"\nComputing alignment metrics ({trials} trials each):")
    print("-" * 50)
    ...
    for metric in AlignmentMetrics.SUPPORTED_METRICS:
        ...
        for t in range(trials):
            ...
            kwargs = {}
            if 'nn' in metric or 'knn' in metric or 'cknna' in metric:
                kwargs['topk'] = 10
            if 'cca' in metric:
                kwargs['cca_dim'] = 10
            if 'kernel' in metric:
        ...
        print(f"{metric.rjust(20)}: {mean_score:.4f} ± {std_score:.4f} [elapsed: {mean_time:.3f}s]")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Compute alignment metrics between two feature files')
    parser.add_argument('--feat-path1', type=str, help='Path to first feature file (.pt)')
    parser.add_argument('--feat-path2', type=str, help='Path to second feature file (.pt)')
    parser.add_argument('--output', type=str, help='Optional output file to save results')
    parser.add_argument('--trials', type=int, default=1, help='Number of trials for each metric')
    ...
    feats_A, feats_B = load_and_process_features(args.feat_path1, args.feat_path2)
    
    print(f"Final feature shapes: {feats_A.shape}, {feats_B.shape}")
    
    # Compute all metrics with multiple trials
    t0 = time.time()
    results = compute_all_metrics(feats_A, feats_B, trials=args.trials)
    ...
    if args.output:
        ...
        save_data = {
            'results': results,
            'feat_file1': args.feat_path1,
            'feat_file2': args.feat_path2,
            'metrics_computed': AlignmentMetrics.SUPPORTED_METRICS,
            'trials': args.trials,
