...
class SILoss:
    ...
    def __call__(self, model, images, model_kwargs=None, zs=None, cls_token=None,
        ...
        # align dtype with model parameters for DeepSpeed
        model_dtype = next(model.parameters()).dtype
        model_input = model_input.to(dtype=model_dtype)
        time_input = time_input.to(dtype=model_dtype)
        cls_input = cls_input.to(dtype=model_dtype)
        ...
        zs_tilde = [z_tilde.to(dtype=torch.float32) for z_tilde in zs_tilde]
        cls_output = cls_output.to(dtype=torch.float32)
